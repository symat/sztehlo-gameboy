...
NEWGAME:
  ...
  while (1) {
    random(255);
    read_all_inputs();
    if (TINYJOYPAD_DOWN || BUTTON_DOWN || TINYJOYPAD_UP) {
      PIC_DRAW(1);
      FIRSTTIME = 1;
      delay(1000);
      break;
...
#define YSTARTSPEED 0
  ...
  while (1) {
    ...
    read_all_inputs();
    if ((ball.y >= 29) && (ball.x >= 18)) {
      if (TINYJOYPAD_DOWN || BUTTON_DOWN) {
        if (SpringBar > 54) { SpringBar = SpringBar - 2; }
      } else {
        if ((SpringBar > SPRINGLONG - 10) && (SpringBar < SPRINGLONG)) { SpringBar = SPRINGLONG; }
        if (SpringBar < SPRINGLONG) { SpringBar = SpringBar + 8; }
      }
    } else {
      SpringBar = SPRINGLONG;
    }
    if (TINYJOYPAD_UP) {
      if (trigFlipG < 3) { trigFlipG++; }
    } else {
      if (trigFlipG > 0) { trigFlipG--; }
    }
    if (TINYJOYPAD_DOWN || BUTTON_DOWN) {
      if (trigFlipD < 3) { trigFlipD++; }
    } else {
      if (trigFlipD > 0) { trigFlipD--; }
    }
  }
}
//<<<<<<<<<<<<<<<<<<<MAIN FIN>>>>>>>>>>>>>>>
...
void SimulateRebounce(uint8_t Sim, BALL *B) {
  ...
  switch (Sim) {
    ...
    case (3):
      ...
      break;
    case (4):
      B->SIMSpeedx = -B->Speedy;
      B->SIMSpeedy = -B->Speedx;
      break;
    case (5):
      B->SIMx = B->x + 1;
      B->SIMy = B->y;
...
void SimulateMove(BALL *B) {
  ...
  if (B->SIMx >= 0) {
    if ((B->SIMSpeedx > -1)) {
      if (B->SIMSpeedx - 0.05 > -1) { B->SIMSpeedx = B->SIMSpeedx - 0.05; }
    }
...
#define BACK (pgm_read_byte(&start[serialcount])&PixelAsign(Y))!=0b00000000)
  if ((X >= 4) && (X <= 9) && (Y >= 11) && (Y <= 14)) {
    if (trigFlipG == 0) {
if ((((pgm_read_byte(&FLIPGAUCHE[(X-4)+(trigFlipG*6)]))&(PixelAsign(Y)))!=0b00000000)||(BACK) {
        return 1;}else{
        return 0;}
    ...
    if (trigFlipD == 0) {
if ((((pgm_read_byte(&FLIPDROITE[(X-4)+(trigFlipD*6)]))&(PixelAsign(Y)))!=0b00000000)||(BACK) {
        return 1;}else{
        return 0;}
...
void PIC_DRAW(uint8_t PIC_) {
  ...
  for (y = 0; y < 8; y++) {
    ...
    for (x = 0; x < 128; x++) {
      switch (PIC_) {
        case (0): TinyOLED_Send(Recup_Intro_PIC(x, y)); break;
        case (1): TinyOLED_Send(SPEED_BLITZ(59, (SSD64x32 == 0) ? 3 : 5, x, y, 0, READY)); break;
        case (2): TinyOLED_Send(SPEED_BLITZ(54, (SSD64x32 == 0) ? 2 : 4, x, y, 0, GameOver)); break;
        default: break;
      }
    }
    TinyOLED_End();
...
void Tiny_Flip(BALL *B) {
  uint8_t m, n;
  for (m = 0; m < 8; m++) {
    TinyOLED_Data_Start(m);
    for (n = 0; n < 128; n++) {
      if ((n > 31) && (n < 96) && (m > 3)) {
        if (n < 90) {
          TinyOLED_Send(addBin(PixelConvert(n, m, B), RecupeByte(n, m), RecupeFlip(n, m), RecupeSpring(n, m)));
        } else {
          TinyOLED_Send(RecupeScreen(n, m));
        }
      } else {
        TinyOLED_Send(0x00);
      }
...
uint8_t PixelConvert(uint8_t horiz, uint8_t verti, BALL *B) {
  ...
  if ((horiz > 31) && (horiz < 97) && (verti > 3)) {
    if ((trunc((bally) / 8) == verti) && (ballx == horiz)) {
      return PixelAsign(bally);
    } else {
      return 0x00;
    }
  } else {
