  ...
  for (y = 0; y < 8; y++) {
    ...
    for (x = 0; x < 128; x++) {
      if (render0_picture1 == 0) {
        if (INGAME) {
          TinyOLED_Send(background(x, y) | SpriteWrite(x, y, Sprite) | DotsWrite(x, y, Sprite) | LiveWrite(x, y) | FruitWrite(x, y));
        } else {
          TinyOLED_Send(0xff - (background(x, y) | SpriteWrite(x, y, Sprite)));
        }
      } else if (render0_picture1 == 1) {
        TinyOLED_Send(pgm_read_byte(&back[x + (y * 128)]));
      }
    }
    TinyOLED_End();
  }
...
uint8_t FruitWrite(uint8_t x, uint8_t y) {
  switch (y) {
    case 7:
      if (x <= 7) { return pgm_read_byte(&fruits[x]); }
      break;
    case 6:
      if ((LEVELSPEED <= 190) && (x <= 7)) { return pgm_read_byte(&fruits[x + 8]); }
      break;
    case 5:
      if ((LEVELSPEED <= 180) && (x <= 7)) { return pgm_read_byte(&fruits[x + 16]); }
      break;
    case 4:
      if ((LEVELSPEED <= 170) && (x <= 7)) { return pgm_read_byte(&fruits[x + 24]); }
      break;
  }
  return 0;
}
